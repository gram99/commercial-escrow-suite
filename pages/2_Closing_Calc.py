...
with st.form("calculator_inputs"):
    ...
    with col_a:
        purchase_price = st.number_input("Purchase Price ($)", min_value=0.0, value=5000000.0, step=10000.0, format="%f")
        earnest_money = st.number_input("Earnest Money Deposit ($)", min_value=0.0, value=250000.0, step=5000.0, format="%f")
        new_loan_amount = st.number_input("New Lender Loan Amount ($)", min_value=0.0, value=3500000.0, step=10000.0, format="%f")
        
    with col_b:
        closing_date = st.date_input("Target Closing Date", datetime.today() + timedelta(days=14))
        annual_tax_bill = st.number_input("Annual Property Tax ($)", min_value=0.0, value=24000.0, step=100.0, format="%f")
    ...
    with col_c:
        title_premium = st.number_input("Title Insurance Premium ($)", min_value=0.0, value=12500.0, step=500.0, format="%f")
        title_premium_payer = st.selectbox("Who pays Title Premium?", ["Split 50/50", "Buyer Pays Entirely", "Seller Pays Entirely"])
        
        escrow_fee = st.number_input("Kensington Vanguard Settlement/Escrow Fee ($)", min_value=0.0, value=2500.0, step=250.0, format="%f")
        escrow_payer = st.selectbox("Who pays Escrow Fee?", ["Split 50/50", "Buyer Pays Entirely", "Seller Pays Entirely"])

    with col_d:
        endorsement_fees = st.number_input("Title Endorsement Charges ($)", min_value=0.0, value=1500.0, step=100.0, format="%f")
        endorsement_payer = st.selectbox("Who pays Endorsements?", ["Buyer Pays Entirely", "Seller Pays Entirely", "Split 50/50"])
        
        recording_fees = st.number_input("Government Recording Fees ($)", min_value=0.0, value=450.0, step=50.0, format="%f")
    ...
    with col_misc2:
        misc_amt = st.number_input("Fee Amount ($)", min_value=0.0, value=0.0, step=100.0, format="%f")
    with col_misc3:
